--- Represents a logical "window" in veek.
-- @parent object
-- @classmod veek-app-window
...
function Object:init(app, display)
	...
	self.gooey = new('veek-container', 1, 1, display.getSize())
	self.gooey.veek_widget.main = app
	self.gooey.veek_widget.fg = 'window-body-fg'
	self.gooey.veek_widget.bg = 'window-body-bg'
end


--- Requests this window be drawed.
function Object:draw()
    self.canvas:set_bg(self.gooey.veek_widget.bg)
    self.canvas:set_fg(self.gooey.veek_widget.fg)
...
function Object:add(widget)
	widget.veek_widget.main = self.app
...
-- @tparam veek-widget widget The widget to select.
