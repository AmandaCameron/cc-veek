...
function Widget:init(w, h)
  self.veek_widget:init(1, 1, w or 1, h or 1)
...
function Widget:scroll_into_view(x, y)
  if not (x - self.scroll_x > 0 and x - self.scroll_x < self.veek_widget.width) then
    self.scroll_x = math.floor(x)

    if self.scroll_x > self.width - self.veek_widget.width then
      self.scroll_x = self.width - self.veek_widget.width
    end
  end
...
function Widget:do_scroll(dir)
  ...
  if self.scroll_y < 0 then
    self.scroll_y = 0
  elseif self.scroll_y > self.height - self.veek_widget.height then
    self.scroll_y = self.height - self.veek_widget.height
  end
end
...
function Widget:do_side_scroll(dir)
  ...
  if self.scroll_x < 1 then
    self.scroll_x = 1
  elseif self.scroll_x > self.width - self.veek_widget.height then
    self.scroll_x = self.width - self.veek_widget.height
  end
end

function Widget:key(k)
  if k == keys.pageUp then
    self:do_scroll(0 - (self.veek_widget.height / 3))

    return true
  elseif k == keys.pageDown then
    self:do_scroll(self.veek_widget.height / 3)
...
function Widget:clicked(x, y, btn)
  ...
  if x == self.veek_widget.width then
    self.moving_y = true

    return true
  elseif y == self.veek_widget.height then
    self.moving_x = true
...
function Widget:dragged(x_delta, y_delta, dir)
  ...
  if self.moving_y then
    self.scroll_y = self.mouse.y * math.floor(self.height / self.veek_widget.height)

    if self.scroll_y > self.height - self.veek_widget.height then
      self.scroll_y = self.height - self.veek_widget.height
    end

    return true
  elseif self.moving_x then
    self.scroll_x = self.mouse.x * math.floor(self.width / self.veek_widget.width)
